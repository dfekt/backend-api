...
var serverManager = function() {
    ...
    var consolelog = function (servers, callback) {
        var response = {}

        servers.forEach(
            function (server) {
                console.log(server)
                var command = util.format("tmux capture-pane -t \"%s:%s.0\" -p", config.server_type, server)
                var buf = execSync(command)
                response[server] = buf.toString('utf8').split('\n').clean("")
        })
